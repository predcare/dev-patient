    ...
    if (Platform.OS === 'android') {
      const granted = await requestAndroidPermission(
        ...
        options || {
          ...
          message: 'App requires access to your camera to capture profile picture.',
        }
      );
      if (!granted) {
    ...
    if (Platform.OS === 'android') {
      ...
      const granted = await requestAndroidPermission(
        ...
        options || {
          ...
          message: 'App requires access to your photos/storage to select media files.',
        }
      );
      if (!granted) {
    ...
    if (Platform.OS === 'android') {
      const granted = await requestAndroidPermission(
        ...
        options || {
          ...
          message: 'App requires access to your location to find nearby services.',
        }
      );
      if (!granted) {
